...
func (d *Dal) CheckKeyAuthorizationRelation(ctx context.Context, kar *qkms_model.KeyAuthorizationRelation) (uint64, error) {
	...
	result := d.Query(ctx).Where("namespace = ? AND name = ? AND environment = ? AND ownerappkey = ? AND grantedappkey = ? And operation = ?", kar.NameSpace, kar.Name, kar.Environment, kar.OwnerAppkey, kar.GrantedAppkey, kar.Behavior).First(&ak)
	if result.Error != nil {
